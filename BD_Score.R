# library(dplyr)
library("glm")
data <- read.csv("20230426_i3_Pipeline.csv")
...
fit.full <- glm(Result ~ Portfolio + i3.Role + Competition.Type + Contract.Type + Customer.Type + Primary.Agency 
           + Contract.Period.Months, data = df2, family=binomial())

# Print the summary of the model to check the results
summary(fit.full)


fit.reduced <- glm(Result ~ Portfolio + i3.Role + Contract.Type + Customer.Type + Contract.Period.Months, data = df2, family=binomial())

fit.reduced <- glm(Result ~ Portfolio + Contract.Type + Customer.Type + Contract.Period.Months, data = df2, family=binomial())
...
newdata = data.frame(Portfolio = "CREWS", Contract.Type = "TBD", Customer.Type = "NB", Contract.Period.Months = 48)
